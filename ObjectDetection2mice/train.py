# -*- coding: utf-8 -*-
"""
Created on Wed March 31 09:28:43 2026
...
def get_model_name():
    ...
    return simpledialog.askstring(
        "Model Name",
        "Enter model name (e.g. BM_pose_2026):"
    )

# ---------------------------
# Fix CSV reading issue
# ---------------------------
BaseTrainer.read_results_csv = lambda self: pl.read_csv(
    self.csv,
...
# Select inputs
# ---------------------------
data_yaml = select_file('Select your dataset YAML (conf_pose.yaml)')
...
model.train(
    ...
    imgsz=640,
    batch=32,              # adjust if needed (16 if OOM)

    device=device,
    amp=True,              # mixed precision → faster GPU

    name=model_name,
    project='U:/LabSoftware/ModelsForTracking_Deep_Learning',

    workers=0,             # parallel data loading
    patience=0,


    # Augmentation (important for your left/right images)
    fliplr=0.5,
    flipud=0.0,
